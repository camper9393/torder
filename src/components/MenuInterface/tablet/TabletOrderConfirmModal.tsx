"use client"

import { createPortal } from "react-dom"
import { Button } from "@/components/ui/button"
import { useAppSelector } from "@/hook/redux"
import { formatPrice } from "@/utils/currency"
import { checkoutLineKey } from "@/utils/menuBilingual"
import { useLocale } from "@/context/LocaleContext"
import { cn } from "@/lib/utils"
import TabletOrderConfirmLine from "./TabletOrderConfirmLine"
import { useTabletCartUiOptional } from "./useTabletCartUi"
import {
  buildTabletThemeCssVars,
  DEFAULT_TABLET_THEME,
} from "@/utils/tabletTheme"

type TabletOrderConfirmModalProps = {
  open: boolean
  submitting: boolean
  onClose: () => void
  onConfirm: () => void
}

function TabletOrderConfirmModal({
  open,
  submitting,
  onClose,
  onConfirm,
}: TabletOrderConfirmModalProps) {
  const { t } = useLocale()
  const cartUi = useTabletCartUiOptional()
  const themeStyle = buildTabletThemeCssVars(cartUi?.theme ?? DEFAULT_TABLET_THEME)
  const checkout = useAppSelector((state) => state.checkOut.items)

  const itemCount = checkout.reduce((sum, item) => sum + item.itemCount, 0)
  const total = checkout.reduce(
    (sum, item) => sum + item.price * item.itemCount,
    0
  )

  if (typeof document === "undefined" || !open) return null

  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      <button
        type="button"
        aria-label={t.common.close}
        className="tablet-themed-modal-backdrop absolute inset-0"
        style={themeStyle}
        onClick={() => !submitting && onClose()}
      />
      <div
        role="dialog"
        aria-modal="true"
        className="tablet-themed-modal-panel relative flex max-h-[min(85vh,760px)] w-full max-w-[560px] flex-col overflow-hidden rounded-2xl shadow-2xl"
        style={themeStyle}
        data-tablet-theme={cartUi?.theme ?? DEFAULT_TABLET_THEME}
      >
        <header className="tablet-themed-modal-header shrink-0 px-5 py-4">
          <h2 className="text-center text-lg font-bold leading-snug md:text-xl">
            {t.tablet.reviewOrderPrompt}
          </h2>
        </header>

        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-2">
          {checkout.length === 0 ? (
            <p className="tablet-themed-muted py-10 text-center text-sm">
              {t.tablet.emptyCart}
            </p>
          ) : (
            <ul>
              {checkout.map((item) => (
                <TabletOrderConfirmLine
                  key={checkoutLineKey(item)}
                  item={item}
                />
              ))}
            </ul>
          )}
        </div>

        <footer className="tablet-themed-modal-footer shrink-0 px-5 py-4">
          <div className="mb-4 space-y-1.5 text-sm">
            <div className="flex justify-between gap-3">
              <span className="tablet-themed-muted font-medium">
                {t.tablet.totalCountLabel}
              </span>
              <span className="font-bold tabular-nums">
                {itemCount}
              </span>
            </div>
            <div className="flex justify-between gap-3">
              <span className="tablet-themed-muted font-medium">
                {t.tablet.totalPriceLabel}
              </span>
              <span className="text-lg font-extrabold tabular-nums">
                {formatPrice(total)}
              </span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <Button
              type="button"
              variant="outline"
              disabled={submitting}
              onClick={onClose}
              className="tablet-themed-btn-secondary min-h-12 rounded-xl text-base font-semibold touch-manipulation"
            >
              {t.tablet.back}
            </Button>
            <Button
              type="button"
              disabled={submitting || checkout.length === 0}
              onClick={onConfirm}
              className={cn(
                "tablet-themed-btn-primary min-h-12 rounded-xl text-base font-bold touch-manipulation disabled:opacity-45"
              )}
            >
              {submitting ? t.tablet.placingOrder : t.tablet.submitOrder}
            </Button>
          </div>
        </footer>
      </div>
    </div>,
    document.body
  )
}

export default TabletOrderConfirmModal
